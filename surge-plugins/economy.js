...
let Economy = global.Economy = {
	...
	// Use Economy function to log uno wins.
	readUnoLadder: function (userid, callback) {
		// In case someone forgot to turn `userid` into an actual ID...
		userid = toId(userid);
		if (userid.substring(0, 5) === 'guest') return 0;
		let amount = Db.unoladder.get(userid, DEFAULT_AMOUNT);
		if (callback && typeof callback === 'function') {
			// If a callback is specified, return `amount` through the callback.
			return callback(amount);
		} else {
	...
	/**
 	* Writes the specified amount of uno ladder wins to the user's "db."
 	* If a callback is specified, the amount is returned through the callback.
 	*
 	* @param {String} userid
 	* @param {Number} amount
 	...
 	* @return {Function} callback (optional)
 	*/
	writeUnoLadder: function (userid, amount, callback) {
		// In case someone forgot to turn `userid` into an actual ID...
		userid = toId(userid);
		if (userid.substring(0, 5) === 'guest') return;
		...
		amount = Number(amount);
		if (isNaN(amount)) {
			throw new Error("Economy.writeUnoLadder: Expected amount parameter to be a Number, instead received " + typeof amount);
		}

		let curTotal = Db.unoladder.get(userid, DEFAULT_AMOUNT);
		Db.unoladder.set(userid, curTotal + amount);
		let newTotal = Db.unoladder.get(userid);
	...
	writeUnoLadderArr: function (users, amount) {
		for (let i = 0; i < users.length; i++) {
			this.writeUnoLadder(users[i], amount);
		}
	},
	// Use Economy function to log tour wins.
	readTourLadder: function (userid, callback) {
		// In case someone forgot to turn `userid` into an actual ID...
		userid = toId(userid);
		if (userid.substring(0, 5) === 'guest') return 0;
		let amount = Db.tourladder.get(userid, DEFAULT_AMOUNT);
		if (callback && typeof callback === 'function') {
			// If a callback is specified, return `amount` through the callback.
			return callback(amount);
		} else {
	...
	/**
 	* Writes the specified amount of tour ladder wins to the user's "db."
 	* If a callback is specified, the amount is returned through the callback.
 	*
 	* @param {String} userid
 	* @param {Number} amount
 	...
 	* @return {Function} callback (optional)
 	*/
	writeTourLadder: function (userid, amount, callback) {
		// In case someone forgot to turn `userid` into an actual ID...
		userid = toId(userid);
		if (userid.substring(0, 5) === 'guest') return;
		...
		amount = Number(amount);
		if (isNaN(amount)) {
			throw new Error("Economy.writeTourLadder: Expected amount parameter to be a Number, instead received " + typeof amount);
		}

		let curTotal = Db.tourladder.get(userid, DEFAULT_AMOUNT);
		Db.tourladder.set(userid, curTotal + amount);
		let newTotal = Db.tourladder.get(userid);
	...
	writeTourLadderArr: function (users, amount) {
		for (let i = 0; i < users.length; i++) {
			this.writeTourLadder(users[i], amount);
		}
	},
