...
class RPSGame {
	...
	parseWin(winner, loser, inactivity) {
		...
		if (this.gameType === "bucks") {
			...
			//do rank change
			let winnerPoints = GL("rpsrank").get(winner.userid, 1000);
			let loserPoints = GL("rpsrank").get(loser.userid, 1000);
			...
			GL("rpsrank").set(winner.userid, winnerFinalPoints);
			...
			GL("rpsrank").set(loser.userid, loserFinalPoints);
...
exports.commands = {
	rps: {
		...
		rank: function (target, room, user) {
			...
			let userRank = GL("rpsrank").get(toId(target), 1000);
		...
		ladder: function (target, room, user) {
			...
			let index = 1;
			let table = Object.keys(GL("rpsrank").object()).sort(function (a, b) {
				if (GL("rpsrank").get(a, 1000) > GL("rpsrank").get(b, 1000)) return -1;
				return 1;
			}).slice(0, 100).map(function (u) {
				return '<tr><td>&nbsp;' + index++ + '&nbsp;</td><td>&nbsp;' + u + '&nbsp;</td><td>&nbsp;' + GL("rpsrank").get(u, 1000) + "&nbsp;</td></tr>";
			}).join("");
