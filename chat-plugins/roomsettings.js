...
class RoomSettings {
	...
	passthebomb() {
		...
		if (this.room.passthebombDisabled) {
			return `${this.button('Pass The Bomb enabled', null, 'Pass The Bomb Enabled')} $(this.button('off', true)}`;
		} else {
			return `${this.button('Pass The Bomb', true)} ${this.button('off', null, 'Pass The Bomb')}`;
		}
	}
	blackjack() {
		...
		if (this.room.blackjackDisabled) {
			return `${this.button('Black Jack enabled', null, 'Black Jack Enabled')} $(this.button('off', true)}`;
		} else {
			return `${this.button('Blacl Jack enabled', true)} ${this.button('off', null, 'Black Jack enable')}`;
		}
	}
	hangman() {
	...
	generateDisplay(user, room, connection) {
		...
		output += `<strong>Black Jack:</strong> <br />${this.blackjack()}<br />`;
		output += '</div>';
