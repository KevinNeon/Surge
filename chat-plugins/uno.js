...
class UNOgame extends Rooms.RoomGame {
	...
	onConnect(user, connection) {
		if (this.state === 'signups') {
			...
			connection.sendTo(this.room, `|uhtml|uno-${this.room.gameNumber}|<div class="uno"><p>A UNO game is currently in progress.</p>${(this.suppressMessages ? `<p style="font-size: 6pt">Game messages will be shown to only players.  If you would like to spectate the game, use <strong>/uno spectate</strong></p>` : '')}</div>`);
		}
	}

	onStart() {
		if (this.playerCount < 2) return false;
		this.sendToRoom(`|uhtmlchange|uno-${this.room.gameNumber}|<div class="uno"><p>The game of UNO has started.</p>${(this.suppressMessages ? `<p style="font-size: 6pt">Game messages will be shown to only players.  If you would like to spectate the game, use <strong>/uno spectate</strong></p>` : '')}</div>`, true);
		this.state = 'play';
	...
	onWin(player) {
		...
		if (this.room.isOfficial) {
			Economy.writeUnoLadder(targetUserid, 1);
